...
public class Painter
{
    ...
    private Path getOrderBookPath(BitcoiniumOrderbook.CondensedOrder[] orders, boolean isFilled)
    {
        ...
        //now plot the line
        for (BitcoiniumOrderbook.CondensedOrder order : orders)
        {
            x = (int) ((order.getPrice().floatValue() - priceLowBound) * priceScalar);
            y = (int) (mainView.getHeight() - order.getVolume().floatValue() * depthScalar);
            p.lineTo(x, y);
        }
    ...
    public void paint(Canvas c)
    {
        ...
        if (okToPaint())
        {//if price scalars are set then data is in...can start to plot
